...
Perchdata = subset(Perchdata, selct= -Species)
```
...
```{r}
best.model <- lm(Weigth~Height+Width,data=Perchdata)
...
Breamdata = subset(Breamdata, selct= -Species)
```
...
Smeltdata = subset(Smeltdata, selct= -Species)
```
